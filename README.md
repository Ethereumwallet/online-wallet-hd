...
Visit [local.ethereumwallet.org:8080](http://local.ethereumwallet.org:8080/).  Do not use `localhost:8080`. You will need to modify your "hosts" file (`/etc/hosts` on OSX and most UNIX systems) because this is no longer registered at the DNS level for application security reasons. Add this line to `/etc/hosts`:

    127.0.0.1   local.ethereumwallet.org
...
http://local.ethereumwallet.org:8080/betaadmin/
...
To reclaim funds from an email take code at the of link and add it to `#/claim`, e.g.:
    ...
    https://ethereumwallet.org/wallet/claim#7Educ5YNnVPQCQ556w7W8tQpj1dchhxPK56vVNab68cK
    http://local.ethereumwallet.org:8080/#/claim/7Educ5YNnVPQCQ556w7W8tQpj1dchhxPK56vVNab68cK
...
There's no UI for this yet, but you can send bitcoins to an email address from the console:
    ...
    Spender(null, function(){console.log("success")}, function(){console.log("fail")},null,null).prepareFromAccount(0, 20000, 10000, function(fromAccount) { fromAccount.toEmail("sjors@ethereumwallet.org") })
...
Protractor UI tests are currently running on https://dev.ethereumwallet.org/ or http://local.ethereumwallet.org. Choose instance in util.js. File with login credentials (ignore.js) will be distributed separately and placed in the `e2e-tests` folder.
