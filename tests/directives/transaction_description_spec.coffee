describe "Transaction Description Directive", ->
  ...
  describe "send to email", ->
    beforeEach ->
      isoScope.transaction.to.account = null
      isoScope.transaction.to.email = {"email":"somebody@ethereumwallet.org"}
    ...
    it "should be known", ->
      pending()
      expect(isoScope.address).toEqual("somebody@ethereumwallet.org")

    it "should be shown", ->
      pending()
      expect(element.html()).toContain 'somebody@ethereumwallet.org'
