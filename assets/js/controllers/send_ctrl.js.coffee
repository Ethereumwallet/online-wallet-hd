walletApp.controller "SendCtrl", ($scope, $log, Wallet, $modalInstance, $timeout, $state, $filter, $stateParams, $translate, paymentRequest, filterFilter, $modal) ->
  ...
  $scope.getBtcCap = () ->
    Wallet.convertFromSatoshi(210000000000000000000000, $scope.btcCurrency)

  $scope.getFiatCap = () ->
    Wallet.convertFromSatoshi(210000000000000000000000, $scope.fiatCurrency)
  ...
  $scope.applyPaymentRequest = (paymentRequest, i) ->
    ...
    console.log("changing amount field: " + (paymentRequest.amount || 0))
  ...
  $scope.send = () ->
    ...
    publish = (passphrase) ->
      $scope.txProposal.publish(passphrase, publicNote, $scope.transaction)
  ...
  $scope.allowedDecimals = (currency) ->
    console.log("Allowed Decimals for currency: " + currency)
  ...
  $scope.checkForSameDestination = () ->
    ...
    transaction.destinations.forEach (dest, index) ->
      $scope.sendForm['destinations' + index].$setValidity('isNotEqual', true)
  ...
  $scope.refreshTxProposal = (recommendCustom=true) ->
    ...
    fee = 50*1000000000*21000
    console.log("UPDATE FROM IN TX PROPOSAL: ");
    console.log(tx.from);
