...
walletServices.factory "Wallet", ($log, $http, $window, $timeout, MyWallet, MyBlockchainApi, MyBlockchainSettings, MyWalletStore, MyWalletSpender, $rootScope, ngAudio, $cookieStore, $translate, $filter, $state, $q) ->
  ...
  wallet.languages = []
  wallet.currencies = []
  wallet.btcCurrencies = [{ serverCode: 'ETH', code: 'ETH', conversion: 1000000000000000000 }]
  ...
  wallet.login = (uid, password, two_factor_code, needsTwoFactorCallback, successCallback, errorCallback) ->
    ...
    betaCheckFinished = () ->
      $window.root = "https://ethereumwallet.org/"
  ...
  wallet.legacyAddresses = () ->
    return wallet.my.wallet.keys if wallet.my.wallet?
    return null
  ...
  wallet.BTCtoFiat = (amount, currency) ->
    ...
    return null unless amount?
    return null unless wallet.conversions[currency]?
    return numeral(1000000000000000000).multiply(amount).divide(wallet.conversions[currency].conversion).format("0.00")

  wallet.isBitCurrency = (currency) ->
    return null unless currency?
    return ['ETH'].indexOf(currency.code) > -1
  ...
  wallet.formatCurrencyForView = (amount, currency) ->
    ...
    code = currency.code
    amount = amount.toFixed(8) if code == 'ETH'
  ...
  wallet.total = (accountIndex) ->
    ...
    switch accountIndex
      when "accounts", undefined, null
        #console.log("loaded balance: " + wallet.my.wallet.hdwallet.balanceActiveAccounts);
      ...
      when "imported"
        ...
        #console.log("did load alt balance: " + account.balance)
  ...
  wallet.fetchExchangeRate = () ->
      ...
      success = (result) ->
        for code, info of result
          ...
          wallet.conversions[code] = {symbol: info.symbol, conversion: parseInt(numeral(1000000000000000000).divide(numeral(info["last"])).format("1"))}
