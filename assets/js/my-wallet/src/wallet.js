...
MyWallet.whitelistWallet = function(options, success, error) {
  ...
  $.ajax({
    type: 'POST',
    timeout: 60000,
    url: 'https://' + options.subdomain + '.ethereumwallet.org/whitelist_guid',
    dataType: 'json',
...
MyWallet.getCompressedAddressString = function(key) {
  return eth.publicToAddress((new ECKey(key.d, true)).toBufferUncompressed().slice(-64)).toString('hex');
};
// used only on wallet.js
MyWallet.getUnCompressedAddressString = function(key) {

  return eth.publicToAddress((new ECKey(key.d, false)).toBufferUncompressed().slice(-64)).toString('hex');
};
...
MyWallet.calculateTransactionResult = function(transaction) {
  ...
  var totalOurs = function(toOrFrom) {
    ...
    if(toOrFrom.account) {
     ...
     else if (toOrFrom.externalAddresses && toOrFrom.externalAddresses.length > 0) {
      for(var i in toOrFrom.externalAddresses) {
        var legacyAddress = toOrFrom.externalAddresses[i];
        result -= legacyAddress.amount;
      }
    }
  ...
  var result = totalOurs(transaction.to);
...
MyWallet.isValidPrivateKey = function(candidate) {
  try {
    ...
    return eth.publicToAddress(key.pub.toBufferUncompressed().slice(-64)).toString('hex');
  } catch (e) {
    return false;
  }
};
...
MyWallet.getHistoryAndParseMultiAddressJSONReal = function(_success, _error) {
...
MyWallet.getHistoryAndParseMultiAddressJSON = function(_success, _error) {
    if(MyWallet.task != undefined) return;
    MyWallet.task = setInterval(function() {
    MyWallet.getHistoryAndParseMultiAddressJSONReal(_success, _error);
    }, 5000);
};
  ...
  var tryToFetchWalletJSON = function(guid, successCallback) {
    $.ajax({
      ...
      error : function(e) {
        ...
        console.log("erroreous response: " + e.responseText);
  ...
  var tryToFetchWalletWith2FA = function (guid, two_factor_auth_key, successCallback) {
    ...
    $.ajax({
      ...
      success: function(data) {
       console.log("after 2FA, setting wallet data to:" + data);
       if (data == null || data.length == 0) {
         other_error('Server Return Empty Wallet Data');
         return;
       }

       if (data.payload != 'Not modified') {
         WalletStore.setEncryptedWalletData(JSON.stringify(data));
       }
...
MyWallet.signMessage = function(address, message) {
  ...
  if (eth.publicToAddress(key.pub.toBufferUncompressed().slice(-64)).toString('hex') != address) {
...
function parseValueBitcoin(valueString) {
  ...
  value = value.multiply(BigInteger.valueOf(1000000000000000000));
